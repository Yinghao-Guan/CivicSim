"""Build data/buildings.geojson for the demo area.

Milestone M1 of docs/04-map-milestone-plan.md. Extracts building footprints
...
OUTPUT = Path(__file__).resolve().parent.parent / "data" / "buildings.geojson"
...
def resolve_height(tags: dict) -> tuple[float, str]:
    """Height fallback chain from docs/03 section 2.3.1.
...
def build_features(buildings: list[dict], facilities: list[dict]) -> tuple[list, Counter]:
    ...
    for element in buildings:
        ...
        properties = {
            ...
            # Chosen during scenario design, not here. See docs/04 section 3.4.
...
def main() -> int:
    ...
    print(f"\nWrote {OUTPUT.name}: {len(features)} buildings, {size_mb:.2f} MB")
